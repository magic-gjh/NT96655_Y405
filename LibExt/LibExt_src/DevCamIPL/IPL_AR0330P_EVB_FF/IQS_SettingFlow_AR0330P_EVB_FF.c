#ifndef _IQS_SETTINGFLOW_AR0330P_EVB_FF_C
#define _IQS_SETTINGFLOW_AR0330P_EVB_FF_C
...
static char     IQS_AR0330_VerInfo[]    = "20130322.06";
    ...
    *iso = ISO;
    ...
    else if ( *iso <= 1000 )
    {
        ...
        else
        {
            *isoL = 600;
            *isoH = 1000;
            *ISO_IdxL =  PRVISO0600B2;
            *ISO_IdxH =  PRVISO0800B2;
        }
    }
    //else if ( *iso <= 1600 )
    else
    {
        ...
        else
        {
            if (AE_getCurLum(8) < 8 )
            {
                *isoL = 1600;
                *isoH = 1600;
                *ISO_IdxL =  PRVLOWLIGHT;
                *ISO_IdxH =  PRVLOWLIGHT;
            }
            else
            {
                *isoL = 1000;
...
static ER SIE_IQparam(IQS_FLOW_MSG MsgID, SIE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    return E_OK;
}
...
ER IQCB_SettingFlowFunc(IQS_FLOW_MSG MsgID, ENG_SUBFUNC *Info, IPL_HAL_GROUP* group)
{
    switch((UINT32)MsgID)
    {
        ...
        case IQCB_PRV_SIE_INIT:
            ...
            SIE_IQparam(MsgID, Info->pSie_sub, group);
            break;
