...
static UINT32 IQS_GetPrvIsoIndex(void)
{
    ...
    if ( IPL_AlgGetUIInfo(IPL_SEL_PRVSENMODE) != SENSOR_MODE_7 && IPL_AlgGetUIInfo(IPL_SEL_PRVSENMODE) != SENSOR_MODE_9 )
    {
        if ( ISO <= 150 )
            IsoIdx =  VQ2ISO0100;
        else if ( ISO <= 300 )
...
static ER IPE_IQparam(IQS_FLOW_MSG MsgID, IPE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    switch((UINT32)MsgID)
    {
        ...
        case IQCB_PREIME_D2D_IPE_QV:
            ...
            //#NT#2013/12/25#Jarkko Chang -begin
            //#NT# switch movie/photo gamma curve: DODLS500W video mode will always use WDR_Gamma
#if 1
            IQ_IPE_RGBGAMMA.GammaTbl_TblAddr = (UINT32)IPE_GammaReorgon((UINT32 *)GammaLUT_128Tab_Mov, (UINT32 *)GammaLUT_128Tab_Mov, (UINT32 *)GammaLUT_128Tab_Mov);
#else
            if ( (IMEffect_Value == SEL_IMAGEEFFECT_WDR) || (Info.Mode == IPL_MODE_VIDEO) || (Info.Mode == IPL_MODE_VIDEOREC))
            {
                IQ_IPE_RGBGAMMA.GammaTbl_TblAddr = (UINT32)IPE_GammaReorgon((UINT32 *)GammaLUT_128Tab_WDR, (UINT32 *)GammaLUT_128Tab_WDR, (UINT32 *)GammaLUT_128Tab_WDR);
            }
            else
            {
...
{
    UINT32 ISO_Value, Zoom_Value, SenMode_Value, IMEffect_Value;
    UINT32 WDR_Value = 0; //IPL_AlgGetUIInfo(IPL_SEL_WDR);
...
void WDR_VideoGainAdjust(SceneRender_RTXGainInfo* GetRtxGainInfo)
{
    ...
    CalRtxInfo.puiIPEGamma = GammaLUT_128Tab_Mov;
    UINT32 ISO, ExpT, Iris;
    AE_GetPrvAEArg(&ISO, &ExpT, &Iris);
...
UINT32 IQS_AddGammaTab(UINT32 Data12Bit)
{
    ...
    YLum = ((GammaLUT_128Tab_Mov[(idx + 1)] - GammaLUT_128Tab_Mov[idx]) * (data - LumIdx)) / (LumIdx1 - LumIdx);
    YLum += GammaLUT_128Tab_Mov[idx];

    if ((YLum < 0) || (YLum > 1024))
    {

        DBG_ERR("%d %d %d %d %d\r\n", idx, LumIdx, LumIdx1, GammaLUT_128Tab_Mov[idx], GammaLUT_128Tab_Mov[(idx + 1)]);
...
UINT32 IQS_InvGammaTab(UINT32 Data10Bit)
{
    ...
    for (i = 1; i < 130; i ++)
    {
        if (Data10Bit <= GammaLUT_128Tab_Mov[i])
        {
            Idx1 = i;
            Idx = (Idx1 - 1);
            ChkFlag = TRUE;
    ...
    if (ChkFlag == TRUE)
    {
        RawLum = (((Idx1 - Idx) * (Data10Bit - GammaLUT_128Tab_Mov[Idx])) << 10) / (GammaLUT_128Tab_Mov[Idx1] - GammaLUT_128Tab_Mov[Idx]);
        RawLum += (Idx << 10);
        RawLum = ((RawLum << 12) >> 7) >> 10;
        if (RawLum >= 4096)
        {
            DBG_ERR("%d %d %d %d %d\r\n", Idx, Idx1, Data10Bit, GammaLUT_128Tab_Mov[Idx], GammaLUT_128Tab_Mov[Idx1]);
        }
        return RawLum;
    }
    else
